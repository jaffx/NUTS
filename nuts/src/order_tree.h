...
public:
    ...
    tree_node<KT, VT> *find_node(vector<KT> &);

    tree_node<KT, VT> *find_node(vector<KT> &&);

    tree_node<KT, VT> *find_node(string path);
...
template<typename KT, typename VT>
tree_node<KT, VT> *order_tree<KT, VT>::find_node(string path) {
...
template<typename KT, typename VT>
tree_node<KT, VT> *order_tree<KT, VT>::find_node(vector<KT> &values) {
    // 找到某个路径上的节点
    auto *p = &root;
